...
#############################################
# NEW: S3 bucket + object for Lambda package
# (Terraform will upload lambda-deployment.zip
#  and Lambda will be created from S3)
#############################################
resource "aws_s3_bucket" "lambda_deploy" {
  bucket = "${local.name_prefix}-lambda-deploy-${data.aws_caller_identity.current.account_id}"
  tags   = local.common_tags
}
...
resource "aws_s3_bucket_ownership_controls" "lambda_deploy" {
  bucket = aws_s3_bucket.lambda_deploy.id
...
resource "aws_s3_object" "lambda_zip" {
  ...
  # Ensures Terraform detects changes and re-uploads when zip changes
  etag = filemd5("${path.module}/../backend/lambda-deployment.zip")

  tags = local.common_tags

  depends_on = [
    aws_s3_bucket_public_access_block.lambda_deploy,
    aws_s3_bucket_ownership_controls.lambda_deploy
  ]
}
...
#############################################
# Lambda function (UPDATED: from S3)
#############################################
resource "aws_lambda_function" "api" {
  ...
  timeout       = 180
  tags          = local.common_tags
  memory_size   = 1024 #new

  # CreateFunction will NOT upload local zip anymore (avoids 413)
  s3_bucket = aws_s3_bucket.lambda_deploy.id
  s3_key    = aws_s3_object.lambda_zip.key

  # Still useful so Terraform detects code changes
